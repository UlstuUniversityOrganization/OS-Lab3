...
char memory_TEST[MEMORY_SIZE];
char backing_store_TEST[BACKING_STORE_SIZE];

int page_table_TEST[COUNT_BACKING_STORE_PAGES][2];
int tlb_table_TEST[TLB_SIZE][2]; 
int claim_table_TEST[COUNT_MEMORY_PAGES][2];
int tlb_size_TEST = 0;
int page_table_size_TEST = 0;
...
int get_frame_by_page_from_tlb(int page) {
    for (int x = 0; x < MIN(tlb_size_TEST, TLB_SIZE); x++) // hit
        if (tlb_table_TEST[x][0] == page) {
            return tlb_table_TEST[x][1];
        }
    return -1;
}

int get_frame_by_page_from_page_tab(int page){
    for (int x = 0; x < MIN(page_table_size_TEST, COUNT_BACKING_STORE_PAGES); x++)
        if (page_table_TEST[x][0] == page) {
            return page_table_TEST[x][1];
        }
    return -1;
}
...
int get_free_ram_page_id() {
    for (int x = 0; x < COUNT_MEMORY_PAGES; x++) // find and claim free frame
        if (!claim_table_TEST[x][0]) {
            return x;
        }
    return -1;
}

void claim_ram_page(int ram_page_id){
    
    printf("keeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeek: %d\n", ram_page_id);
    // printf(ram_page_id);
    // printf("\n");
    claim_table_TEST[ram_page_id][0] = 1;
}

void add_page_to_page_table(int page_id, int frame_id) {
    
    page_table_TEST[page_table_size_TEST % COUNT_BACKING_STORE_PAGES][0] = page_id;
    page_table_TEST[page_table_size_TEST % COUNT_BACKING_STORE_PAGES][1] = frame_id;
    page_table_size_TEST++;
    
    tlb_table_TEST[tlb_size_TEST % TLB_SIZE][0] = page_id;
    tlb_table_TEST[tlb_size_TEST % TLB_SIZE][1] = frame_id;
    tlb_size_TEST++;
...
int find_least_used_ram_page_id() {
    int use_frequency = claim_table_TEST[0][1];
    int least_used_ram_page_id = 0;
    for (int x = 1; x < COUNT_MEMORY_PAGES; x++) // find the least used frame
        if (claim_table_TEST[x][1] <= use_frequency) {
            use_frequency = claim_table_TEST[x][1];
...
int get_tlb_row_id_by_frame(int frame) {
    for (int x = 0; x < MIN(tlb_size_TEST, TLB_SIZE); x++)
        if (tlb_table_TEST[x][1] == frame) {
            return x;
        }
    return -1;
}
...
void delete_row_from_page_table(int row_id){
    // 1, 2, 3, 4, e, 5, 6
    ...
    // 1, 2, 3 ,4, 6, 5, _
    memcpy(page_table_TEST[row_id], page_table_TEST[(page_table_size_TEST - 1) % COUNT_BACKING_STORE_PAGES], 2);
    page_table_size_TEST--;
    // page_table_size is a pointer to the last added element, not the overall count of elements.
}

void delete_row_from_tlb(int row_id){
    memcpy(tlb_table_TEST[row_id], tlb_table_TEST[(tlb_size_TEST - 1) % TLB_SIZE], 2);
    tlb_size_TEST--;
}
...
int virtual_to_physical(int virtual_address) {
    ...
    if (frame < 0){ // the frame wasn't found in tlb
        ...
        if (frame < 0){ // the frame wasn't found in page table
            ...
            memcpy(memory_TEST + free_ram_page_id * PAGE_SIZE, backing_store + page * PAGE_SIZE, PAGE_SIZE);
...
int main(int argc, char* argv[]) {
    ...
    memset(page_table_TEST, -1, sizeof(int) * COUNT_BACKING_STORE_PAGES * 2);
    memset(tlb_table_TEST, -1, sizeof(int) * TLB_SIZE * 2);
    memset(claim_table_TEST, 0, sizeof(int) * COUNT_MEMORY_PAGES * 2);

    claim_table_TEST[244][0] = 1;
    printf("lololol\n");
    ...
    while (fscanf(addreses, "%d", &virtual_address) == 1) {
        ...
        int phys_addr = virtual_to_physical(virtual_address);
        // int value = memory_TEST[phys_addr];
